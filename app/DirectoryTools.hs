...
flatFilesystemTree' root node [] = do return ()
...
cleanDirectory' (x:xs) root 
    | (takeExtension' x == ".jpg" ||  takeExtension' x == ".jpeg")                                                                                                                                    = toFolder "IMG"  
...
conditionalMoveInto pattern folderName root = do
    c <- getAllContents root
    isDir <- doesDirectoryExist root
    let targetFolder = root ++ "/" ++ folderName
    targetExist <- doesDirectoryExist targetFolder
    when (not targetExist) (createDirectory targetFolder)
    let filterList = Data.List.filter (\x -> isInfixOf (pack pattern) (pack $ takeFileName x)) c
    if (isDir)
        then conditionalMoveInto' filterList folderName root
        else return ()
